...
import type { Area, Despesa } from './types'
import { getCategoria } from './categories'
...
export function gerarRelatorioPDF(areas: Area[], despesas: Despesa[]) {
  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' })
  const hoje = new Date().toLocaleDateString('pt-BR')
  const largura = doc.internal.pageSize.getWidth()

  // ── Cabeçalho ──────────────────────────────────────────────
  doc.setFillColor(22, 163, 74) // verde primário
